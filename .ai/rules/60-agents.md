...
### Agent Rules
- Agents must not directly access databases or external APIs unless through a tool layer.
...
### Tooling
- Every tool should have: a clear purpose, input schema, output schema, and failure behavior.
...
- Store long-term memory only when justified by product requirements.

### Do Not
- Let agents perform unrestricted actions.
- Hide tool errors from logging or monitoring.
- Mix prompt text deeply into business logic code.
